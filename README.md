...
۲. **فریم‌های بدون تغییر اصلاً وارد خط لوله نمی‌شوند** — در یک بازی ۶۰ فریمی، بیش از ۹۵٪ فریم‌ها دور ریخته می‌شوند پیش از آنکه به OCR برسند.
   ...
   مقایسه روی «امضای متن» انجام می‌شود نه خود تصویر: فقط پیکسل‌هایی که هم لبه‌ی تیز محلی دارند هم روشن‌اند. چون بیشتر بازی‌ها زیرنویس را مستقیم روی صحنه می‌کشند، مقایسه‌ی خام تصویر با هر تکان دوربین شلیک می‌کند. اندازه‌گیری‌شده روی صحنه‌ی در حال حرکت که متن یک بار عوض می‌شود:

   | ناحیه | مقایسه‌ی خام تصویر | امضای متن |
   |---|---|---|
   | نوار ۱۵۵۰×۱۲۰ | ۶۰ از ۶۰ فریم | **۲ از ۶۰** |
   | یک‌سوم پایین ۱۹۲۰×۳۶۰ | ۶۰ از ۶۰ | **۲ از ۶۰** |
   | نصف پایین ۱۹۲۰×۵۴۰ | ۶۰ از ۶۰ | **۲ از ۶۰** |

   هر «شلیک» یعنی یک فراخوانی OCR و احتمالاً یک درخواست API.
...
#### اگر GPU پشتیبانی‌شده ندارید

Ollama استاندارد فقط NVIDIA و AMD را شتاب می‌دهد. روی گرافیک یکپارچه‌ی Intel یا بدون GPU، مدل **روی CPU** اجرا می‌شود و سرعت تقریباً با اندازه‌ی مدل نسبت عکس دارد. دو کار کمک می‌کند:

۱. **مدل کوچک‌تر** — یک مدل ۴B معمولاً حدود دو برابر سریع‌تر از ۸B است. `gametrans compare-models` (منوی ۷) تفاوت را روی سخت‌افزار خودتان نشان می‌دهد.

۲. **برای گرافیک Intel Arc**: [IPEX-LLM](https://github.com/intel/ipex-llm/blob/main/docs/mddocs/Quickstart/ollama_portable_zip_quickstart.md) یک نسخه‌ی جایگزین Ollama می‌دهد که از iGPU اینتل استفاده می‌کند. (NPU فعلاً پشتیبانی نمی‌شود.)

تنظیمات مربوط به سرعت در `[[translate.providers]]` مدل لوکال هستند: `num_ctx` کوچک‌تر، `num_thread` برابر تعداد هسته‌های اصلی، و `compact_prompt = true`.
...
**۳. متن خیلی سریع می‌رود و نمی‌رسم بخوانم؟**

اورلی چند خط آخر را روی هم نگه می‌دارد. از منو گزینه‌ی **۸** یا مستقیم در `config.toml`:

```toml
[overlay]
history_lines = 4      # چند خط اخیر هم‌زمان بمانند
linger_ms     = 4000   # هر خط چند میلی‌ثانیه بماند
history_dim   = 0.55   # خطوط قدیمی‌تر چقدر کم‌رنگ شوند
```

خط جدید پررنگ است و قبلی‌ها کم‌رنگ‌تر، پس همیشه معلوم است کدام «الان» است.

**۴. ناحیه را چقدر بزرگ بگیرم؟**

تا «یک‌سوم پایین صفحه» راحت جواب می‌دهد و جای زیرنویس در بازی‌های مختلف را پوشش می‌دهد. **کل صفحه را انتخاب نکنید** — نه به خاطر دروازه‌ی تغییر (که حالا مقاوم است)، بلکه چون OCR روی کل صفحه کند است و نوار سلامتی، مینی‌مپ، تعداد فشنگ و هر متن دیگری را هم می‌خواند و ترجمه می‌کند؛ سهمیه‌ی رایگان چند ثانیه‌ای تمام می‌شود و اورلی پر از آشغال.

اگر زیرنویس بازی کم‌نور است و تشخیص داده نمی‌شود، در `config.toml`:

```toml
[capture]
mask_bright = 130.0   # پیش‌فرض ۱۷۰
```

**۵. زیرنویس‌ها دیر تشخیص داده می‌شوند؟** آستانه‌ی تغییر را کم کنید:
